...
import os
#from bentley_muscat.items import BentleyMuscatItem, MetaItem
from autodata.items import AutodataItem, MetaItem

class BentleySpider(scrapy.Spider):
    name = 'bentley_muscat'
    allowed_domains = []
    start_urls = ['https://muscat.bentleymotors.com/meia/en/stock/']

    def parse(self, response):
        sel = Selector(response)
        path = sel.xpath('//*[@id="results"]/article')
        for p in path:
            url = "https://muscat.bentleymotors.com" + ''.join(p.xpath('.//div[@class = "column block"]/a[@class="mdx-gtm-srp-open-vdp button btn-primary details"]/@href').extract()).strip()
