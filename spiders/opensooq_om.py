...
class OpensooqOmSpider(scrapy.Spider):
    name = 'opensooq_om'
    allowed_domains = []
    start_urls = ['https://om.opensooq.com/en/cars/cars-for-sale']

    def parse(self,response):
        cars = response.xpath('//div[@class="post-item"]/ul/li[@class="rectLi ie relative mb15"]//h2[@class="fRight mb15"]')
        for car in cars:
            url = 'https://om.opensooq.com' + ''.join(car.xpath('a/@href').extract()).strip()
        ...
        next_url = 'https://om.opensooq.com' + url
        if url != '':
