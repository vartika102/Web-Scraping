...
#from chevrolet_qatar.items import ChevroletQatarItem, MetaItem
from autodata.items import AutodataItem, MetaItem


class ChevroletSpider(scrapy.Spider):
    name = 'chevrolet_qatar'
    allowed_domains = []
    start_urls = ['https://www.chevroletqatar.com/pre-owned-vehicles//']

    def parse(self, response):
        sel = Selector(response)
        yield Request(response.url,callback= self.parse_url,)
        nextt = sel.xpath('//div[@class="pagination"]/div/ul/li[@class="btn-next"]')
        urll = "https://www.chevroletqatar.com/pre-owned-vehicles/" + ''.join(nextt.xpath('a[@title="next page"]/@href').extract()).strip()
        if(nextt != []):
            print("yess")
            urll = "https://www.chevroletqatar.com/pre-owned-vehicles/" + ''.join(nextt.xpath('a[@title="Next Page"]/@href').extract()).strip()
    ...
    def parse_url(self, response):
        ...
        for url in urls:
            new_url = "https://www.chevroletqatar.com" + ''.join(url.xpath('.//a[@title="Vehicle Details"]/@href').extract())
            yield Request(new_url,callback=self.parse_data,)     
        pass

    def parse_data(self,response):

##        item1 = ChevroletQatarItem()
##        item2 = MetaItem()
        ...
        item1['Year'] = ''.join(details2.xpath('.//div[@class="cell reg-year"]/span[@class="value reg-year"]//text()').extract()).strip()         
        ...
        item1['asking_price_inc_VAT'] = (''.join(details4.xpath('span[@class="value"]//text()').extract()).strip())[4:]

        item2['src'] = "chevroletqatar.com"
        item2['ts'] = datetime.utcnow().isoformat()
        item2['name'] = "chevrolet"
        ...
        item1['Seller_Name'] = 'Jaidah Group'
