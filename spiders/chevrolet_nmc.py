...
class ChevroletNmcSpider(scrapy.Spider):
    name = 'chevrolet_nmc'
    allowed_domains = []
    start_urls = ['http://chevrolet.nmc.com.bh/pre-owned-vehicles/']
    ...
    def parse(self, response):
        ...
        urll = "http://chevrolet.nmc.com.bh/pre-owned-vehicles/" + ''.join(nextt.xpath('a[@title="next page"]/@href').extract()).strip()
        if(nextt != []):
            #print("yess")
            urll = "http://chevrolet.nmc.com.bh/pre-owned-vehicles/" + ''.join(nextt.xpath('a[@title="Next Page"]/@href').extract()).strip()
    ...
    def parse_url(self, response):
        ...
        for url in urls:
            new_url = "http://chevrolet.nmc.com.bh" + ''.join(url.xpath('.//a[@title="Vehicle Details"]/@href').extract())
    ...
    def parse_data(self,response):
        ...
        sel = Selector(response)
        details1 = sel.xpath('//div[@class="row-fluid row-nkw6p"]//div[@class = "title module"]')
        ...
        item2['src'] = "chevrolet.nmc.com"
        item2['ts'] = datetime.utcnow().isoformat()
        item2['name'] = "chevrolet_nmc"
        ...
        item1['Seller_Name'] = 'National Motor Company'
