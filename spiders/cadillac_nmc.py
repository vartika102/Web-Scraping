...
class CadillacSpider(scrapy.Spider):
    ...
    allowed_domains = []
    urls = []
    start_urls = ["http://cadillac.nmc.com.bh/pre-owned-vehicles/"]
    
    
    
    def parse(self,response):
        for href in response.xpath("//div[contains(@class, 'title module')]/h3/a//@href"):
            url="http://cadillac.nmc.com.bh"+href.extract()
    ...
    def parse_dir_contents(self,response):
        ...
        item["Seller_Name"] = "Cadillac National Motor Company"
        ...
        item2['src'] = "cadillac.nmc.com"
        item2['ts'] = datetime.utcnow().isoformat()
        item2['name'] = "cadillac_nmc"
        ...
        item['asking_price_inc_VAT'] = (response.xpath("//div[contains(@class,'price-now')]/span[contains(@class,'value')]/text()").extract()[0]).split('BHD')[-1].strip()
        ...
        item['mileage'] = ''.join(response.xpath("//div[contains(@class, 'cell mileage')]/span[contains(@class, 'value mileage')]/text()").extract()).strip().split(' ')[0]
        item['mileage_unit'] = ''.join(response.xpath("//div[contains(@class, 'cell mileage')]/span[contains(@class, 'value mileage')]/text()").extract()).strip().split(' ')[1]
