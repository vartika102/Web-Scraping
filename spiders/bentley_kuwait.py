...
class BentleySpider(scrapy.Spider):
    name = 'bentley_kuwait'
    allowed_domains = []
    start_urls = ['https://kuwait.bentleymotors.com/meia/en/stock']

    def parse(self, response):
        sel = Selector(response)
        path = sel.xpath('//*[@id="results"]/article')
        for p in path:
            url = "https://kuwait.bentleymotors.com" + ''.join(p.xpath('.//div[@class = "column block"]/a[@class="mdx-gtm-srp-open-vdp button btn-primary details"]/@href').extract()).strip()
    ...
    def parse_data(self, response):
        ...
        item1["City"] = ""
        item1["Seller_Type"] = ""
        item1["Seller_Name"] = "Bentley Motors Kuwait"
        item1["Car_URL"] = ""
        item1["Car_Name"] = ""
        item1["Year"] = ""
        item1["Make"] = ""
        ...
        item1["Price_Currency"] = ''.join(sel.xpath('//div[@class="vehicle-prive column block"]/div//text()').extract()).strip().split(' ')[1]
        item1["asking_price_inc_VAT"] = ''.join(sel.xpath('//div[@class="vehicle-prive column block"]/div//text()').extract()).split(' ')[0]
        ...
        for l in lis:
            ...
            if key.lower() == "body style":
                ...
                item1["other_specs_horse_power"] = int(item.replace(u'\xa0', u' ').replace(',','').split(' ')[0]) * 1.34102
