...
class LandroverSpider(Spider):
    name = 'landrover'
    start_urls = ['http://approved.me.landrover.com/en_qa/used/saudi-arabia?_ga=2.194035247.363860144.1556196830-1857024885.1556196830']
    urls= ['http://approved.me.landrover.com/en_qa/used/saudi-arabia?_ga=2.194035247.363860144.1556196830-1857024885.1556196830']
    ...
    def parse(self, response):
        body = response.body
        links = response.xpath("//div[contains(@class,'results__vehicle small-12  medium-4 large-3 ')]/a[1]/@href").extract()
        links = list(OrderedDict.fromkeys(links))
        print(len(links))
        for link in links:
            web= "http://approved.me.landrover.com"+link
    ...
    def getdata(self,response):
        ...
        item2['src'] = "approved.me.landrover.com"
        item2['ts'] = datetime.utcnow().isoformat()
        item2['name'] = "landrover"
        ...
        item["Car_Name"] = item["Make"] + ' ' + ''.join(response.xpath('//hgroup/h1[@class="section-title"]/text()').extract()).replace(arr[c].upper() + ' ','').strip()
