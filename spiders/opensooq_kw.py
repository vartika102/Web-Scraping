...
class OpensooqKwSpider(scrapy.Spider):
    name = 'opensooq_kw'
    allowed_domains = []
    start_urls = ['https://kw.opensooq.com/en/cars/cars-for-sale']

    def parse(self, response):
        cars = response.xpath('//div[@class="post-item"]/ul/li[@class="rectLi ie relative mb15"]//h2[@class="fRight mb15"]')
        for car in cars:
            url = 'https://kw.opensooq.com' + ''.join(car.xpath('a/@href').extract()).strip()
        ...
        next_url = 'https://kw.opensooq.com' + url
        if url != '':
    ...
    def parse_data(self, response):
        ...
        for det in details:
            ...
            if 'city' in key.lower():
                ...
                if '-' in value:
                    item1["mileage"] = value.split('-')[1].strip()
                else:
                    item1["mileage"] = value
                item1["mileage"] = item1["mileage"].replace('+','')
        ...
        item2['src'] = "kw.opensooq.com"
        item2['ts'] = datetime.utcnow().isoformat()
        item2['name'] = "opensooq_kw"
